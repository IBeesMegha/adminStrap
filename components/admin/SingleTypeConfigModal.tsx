...
interface SingleTypeConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
  singleType: {
    name: string;
...
export const SingleTypeConfigModal: React.FC<SingleTypeConfigModalProps> = ({
  isOpen,
  onClose,
  singleType,
}) => {
  ...
  const apiRoutes = {
    get: {
      method: 'GET',
      path: `/api/single-types/${singleType.name}`,
      description: 'Get single type entry',
      example: `${baseUrl}/api/single-types/${singleType.name}`,
    },
    update: {
      method: 'PUT',
      path: `/api/single-types/${singleType.name}`,
      description: 'Update single type entry',
      example: `${baseUrl}/api/single-types/${singleType.name}`,
    },
    delete: {
      method: 'DELETE',
      path: `/api/single-types/${singleType.name}`,
      description: 'Delete single type entry',
      example: `${baseUrl}/api/single-types/${singleType.name}`,
    },
  };
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {singleType.displayName} Configuration
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Single Type: {singleType.name}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex items-start">
                ...
                <span className="text-sm text-gray-900 flex-1">{singleType.name}</span>
              </div>
              <div className="flex items-start">
                <span className="text-sm font-medium text-gray-600 w-32">Display Name:</span>
                <span className="text-sm text-gray-900 flex-1">{singleType.displayName}</span>
              </div>
              {singleType.description && (
                <div className="flex items-start">
                  <span className="text-sm font-medium text-gray-600 w-32">Description:</span>
                  <span className="text-sm text-gray-900 flex-1">{singleType.description}</span>
                </div>
              )}
            </div>
          </div>
