...
export const CKEditorField: React.FC<CKEditorFieldProps> = ({
  ...
  useEffect(() => {
    ...
    const initEditor = async () => {
      try {
        ...
        (window as any).__CKEditorModulesEditor = {
          CKEditor,
          ClassicEditor,
          plugins: [
            Essentials,
  ...
  if (!isLoaded || typeof window === 'undefined') {
    return (
      <div 
        className={`w-full border border-gray-300 rounded-lg bg-gray-50 flex items-center justify-center ${className}`}
  ...
  const modules = (window as any).__CKEditorModulesEditor;
  if (!modules) {
    return (
